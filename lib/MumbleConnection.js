...
MumbleConnection.prototype.sendMessage = function (type, data) {
    ...
    var packet = Messages.buildPacket(type, data);
...
MumbleConnection.prototype._onUDPTunnel = function( data ) {
    ...
    var session = util.toVarint( this.sessionId );
...
/*    this.socket.write(prefix);
    ...
    this.emit( 'broadcast_audio', prefix, this.sessionId );

    // Write the voice header
    this.emit( 'broadcast_audio', voiceHeader, this.sessionId );
    this.emit( 'broadcast_audio', packet, this.sessionId );
};
